...
--
-- CompilerD.lua
-- This Script contains the new CompilerD
...
local OPAQUE_PREDICATE_PROBABILITY = 0.5;        -- 50% of the time
local MAX_REGISTERS = 5                          -- 5 registers
local RARELY_CHECKED_CONDITION_PROBABILITY = 0.3 -- 10% of the time
local cflowFlatteningProbability = 0.3           -- 50% of the time

local INSTRUCTION_SUBSTITUTION_PROBABILITY = 0.3 -- 50% of the time
local DEAD_CODE_PATH_PROBABILITY = 0.3           -- 30% of the time
local INSTRUCTION_JUMBLINING_PROBABILITY = 0.3   -- 50% of the time


local CompilerD = {}
CompilerD.Name = "Compiler D"

local Ast = require("PrometheusFork.ast");
local Scope = require("PrometheusFork.scope");
local logger = require("logger");
local util = require("PrometheusFork.util");
local visitast = require("PrometheusFork.visitast")
local randomStrings = require("PrometheusFork.randomStrings")
local ControlFlowObfuscator = require("PrometheusFork.ControlFlowGen")
...
function CompilerD:new(maxRegs)
...
function CompilerD:adjustRegisters(complexity)
...
function CompilerD:substituteInstruction(op, a, b, complexityFactor)
...
function CompilerD:generateDeadCodePath(dynamicData)
...
function CompilerD:reorderStatements(statements, complexityFactor)
    local graph = {}
...
-- Step 1: Complex Mathematical Operations
function CompilerD:generateComplexMathOperation()
    local x, y, z = math_random(1, 100), math_random(1, 100), math_random(1, 100)
    local a, b, c = math_random(1, 100), math_random(1, 100), math_random(1, 100)
    local complexMathOperation = string.format("((%d + %d) * %d) / %d - %d + %d", x, y, z, a, b, c)
    return complexMathOperation
end

-- Step 2: Incorporate Runtime Data
function CompilerD:incorporateRuntimeData()
    local currentTime = os.time()
    local runtimeData = string.format("Runtime data: %d", currentTime)
    return runtimeData
end

-- Step 3: Combine Multiple Conditions (icorperateRuntimeData and generateComplexMathOperation)
function CompilerD:createComplexOpaquePredicate()
    local complexOpaquePredicate = {}
    for i = 1, math_random(2, 4) do
        local condition = math_random() < 0.5 and self:generateComplexMathOperation() or self:incorporateRuntimeData()
        table.insert(complexOpaquePredicate, condition)
    end
    return table.concat(complexOpaquePredicate, " and ")
end

-- Control Flow Flattening/Obfuscation (Cflow)
function CompilerD:createBlock(parentBlock, condition, dynamicData, nestingLevel)
    ...
    local opaquePredicate = self:createComplexOpaquePredicate()

    -- Memory Layout Randomization: Introduce dummy variables with unpredictable names and randomize the order
    local randomizedOrder = { obfuscatedCondition, opaquePredicate }
    for i = 1, math_random(2, 3) do
        local dummyVariable = "__" .. math_random(1, 100) .. "__"
        table.insert(randomizedOrder, dummyVariable)
    end
    randomizedOrder = self:shuffle(randomizedOrder)
    ...
    local block = {
        ...
        condition = randomizedOrder[1],
        opaquePredicate = randomizedOrder[2],
    }
    ...
    local complexityFactor = dynamicData and #tostring(dynamicData) % 10 or 1
    for i = 1, math_random(2, 3) * complexityFactor do
        if math_random() < DUMMY_BLOCK_PROBABILITY then
            ...
            local obfuscatedDummyCondition = obfuscator:generateNestedObfuscatedConditionWithRarelyCheckedConditions(dummyComplexity)

            local dummyOpaquePredicate = (self.idCounter % 2 == 0)

            -- Memory Layout Randomization for dummy blocks
            local dummyRandomizedOrder = { obfuscatedDummyCondition, dummyOpaquePredicate }
            for i = 1, math_random(2, 3) do
                local dummyVariable = "__" .. math_random(1, 100) .. "__"
                table.insert(dummyRandomizedOrder, dummyVariable)
            end

            dummyRandomizedOrder = self:shuffle(dummyRandomizedOrder)
            ...
            local dummyBlock = {
                ...
                condition = dummyRandomizedOrder[1],
                opaquePredicate = dummyRandomizedOrder[2],
            }
    ...
    if selectedStructure == "while" or selectedStructure == "for" or selectedStructure == "repeat-until" then
        ...
        for j = 1, math_random(2, 4) * complexityFactor do
            ...
            -- Loop Unrolling: Expanding loops into repeated code blocks
            for k = 1, math_random(2, 4) do
                ...
                -- Nested loop unrolling for added complexity
                for l = 1, math_random(2, 4) do
                    local nestedCondition = obfuscator:generateNestedObfuscatedConditionWithRarelyCheckedConditions(complexity + l)
                    table.insert(loopBlock.body, {
                        op = "if",
                        condition = nestedCondition,
                        thenBlock = mutatedNumber + l,
                        elseBlock = (mutatedNumber + l + 1) % #self.registers,
                        inliningCondition = obfuscator:inlining(),
                        eqmutateCondition = obfuscator:eqmutate(),
                        bounceCondition = obfuscator:bounce()
                    })
                end
            end
        end
    ...
    elseif selectedStructure == "switch" then
        ...
        for k = 1, math_random(2, 4) * complexityFactor do
    ...
    if nestingLevel and nestingLevel > 0 then
        for i = 1, math_random(1, 3) * complexityFactor do
    ...
    if math_random() < DEAD_CODE_PATH_PROBABILITY then
        local deadCodeBlock = {
            op = "dead_code",
            statements = {
                self:generateDeadCodePath(dynamicData)
            }
        }
        table.insert(block.statements, deadCodeBlock)
    end
    ...
    if math_random() < cflowFlatteningProbability and #self.registers > 0 then
        local condition = obfuscator:generateNestedObfuscatedConditionWithRarelyCheckedConditions(obfuscator:numberMutation())
        for i = 1, math_random(2, 4) * complexityFactor do
    ...
    -- Instruction substitution based on probability
    for i, statement in ipairs(block.statements) do
        if math_random() < INSTRUCTION_SUBSTITUTION_PROBABILITY then
            local op = statement.op
            local a = statement.a
            local b = statement.b
            statement.op = self:substituteInstruction(op, a, b, complexityFactor)
        end
    end

    if math_random() < INSTRUCTION_JUMBLINING_PROBABILITY then
        local jumbledStatements = {}
        for i, statement in ipairs(block.statements) do
            table.insert(jumbledStatements, statement)
            table.insert(jumbledStatements, self:generateComplexMathOperation())
            table.insert(jumbledStatements, self:incorporateRuntimeData())
            table.insert(jumbledStatements, self:generateDeadCodePath(dynamicData))
            table.insert(jumbledStatements, self:createComplexOpaquePredicate())
        end
        block.statements = jumbledStatements
    end
    ...
    local functionTable = {
        self.createComplexOpaquePredicate,
        self.generateDeadCodePath,
        obfuscator.generateNestedObfuscatedConditionWithRarelyCheckedConditions,
        obfuscator.inlining,
        obfuscator.eqmutate,
        obfuscator.bounce,
        obfuscator.numberMutation
    }
    -- Randomly shuffle the function table to change the order of execution
    for i = #functionTable, 2, -1 do
        local j = math_random(i)
        functionTable[i], functionTable[j] = functionTable[j], functionTable[i]
    end
    -- Execute the functions in the shuffled order
    block.dynamicDispatch = function(self)
        for i, func in ipairs(functionTable) do
            func()
        end
    end
...
function CompilerD:shuffle(array)
    local n = #array
    for i = n, 2, -1 do
...
function CompilerD:setActiveBlock(block)
    self.activeBlock = block;
end

function CompilerD:addStatement(statement, writes, reads, usesUpvals)
...
function CompilerD:compile(ast)
    self.blocks               = {};
    self.registers            = {};
...
function CompilerD:getCreateClosureVar(argCount)
...
function CompilerD:pushRegisterUsageInfo()
...
-- Control Flow Flattening/Obfuscation (Cflow)
function CompilerD:createFreeUpvalueFunc()
    local obfuscator = ControlFlowObfuscator:new()
    self.idCounter = (self.idCounter or 0) + 1
    local id = self.idCounter
    ...
    for i = 1, 10 do
        local dynamicCondition = obfuscator:generateDynamicCondition(math_random(1, 10))
        for j = 1, 5 do
            local nestedDynamicCondition = obfuscator:generateDynamicCondition(math_random(1, 10))
            if nestedDynamicCondition then
                block.statements[#block.statements + 1] = {
                    op = "if",
                    condition = nestedDynamicCondition,
                    thenBlock = j,
                    elseBlock = (j + 1) % 5,
                }
                for k = 1, 3 do
                    local tripleNestedCondition = obfuscator:generateDynamicCondition(math_random(1, 10))
                    if tripleNestedCondition then
                        block.statements[#block.statements + 1] = {
                            op = "if",
                            condition = tripleNestedCondition,
                            thenBlock = k,
                            elseBlock = (k + 1) % 3,
                        }
                    end
    ...
    if math_random() < cflowFlatteningProbability then
        ...
        block.selectBlock = function(self, input)
            ...
            return self.switchCase[index + conditionCheck % #self.switchCase]
        end

        for i, randomNum in ipairs(randomValues) do
            local complexCondition = obfuscator:generateNestedObfuscatedCondition(randomNum)
            if complexCondition and math_random() > 0.5 then
                for j = 1, loopLimit do
                    local nestedCondition = obfuscator:generateNestedObfuscatedConditionWithRarelyCheckedConditions(
                        randomNum * j)
                    if nestedCondition or j % 2 == 0 then
                        for k = 1, numConditions do
                            if k % 3 == 0 then
                                block.statements[#block.statements + 1] = {
                                    op = "if",
                                    condition = self:createDeeplyNestedConditions(obfuscator, randomValues[k]),
                                    thenBlock = k,
                                    elseBlock = (k + 1) % numConditions,
                                }
                            else
                                block.statements[#block.statements + 1] = {
                                    op = "else",
                                    condition = self:createDeeplyNestedConditions(obfuscator, randomValues[k] + 1),
                                    thenBlock = k,
                                    elseBlock = (k + 2) % numConditions,
                                }
                            end
                        end
                    end
    ...
    if math_random() < DEAD_CODE_PATH_PROBABILITY then
        ...
        block.statements[#block.statements + 1] = deadCodeBlock
    end

    if math_random() < INSTRUCTION_SUBSTITUTION_PROBABILITY then
        local op = self:substituteInstruction()
        block.statements[#block.statements + 1] = {
            op = op,
            a = math_random(1, 255),
            b = math_random(1, 255)
    ...
    local dynamicControlFlowCount = math_random(3, 10)
    for i = 1, dynamicControlFlowCount do
        local dynamicCondition = obfuscator:generateDynamicCondition(math_random(1, 10))
        local dynamicBlock = {
            op = "if",
            condition = dynamicCondition,
            thenBlock = i,
            elseBlock = (i + 1) % dynamicControlFlowCount

        }
        block.statements[#block.statements + 1] = dynamicBlock
    end
    ...
    for i = 1, #block.statements do
        if i % 4 == 0 then
            local dynamicCondition = obfuscator:generateDynamicCondition(math_random(1, 10))
            block.statements[i].condition = dynamicCondition
            if dynamicCondition and math_random() > 0.7 then
                block.statements[i].nested = {
                    op = "if",
                    condition = self:createComplexOpaquePredicate(),
                    thenBlock = i,
                    elseBlock = (i + 1) % #block.statements,
                }
            end
        end
    end
...
function CompilerD:createDeeplyNestedConditions(obfuscator, seed)
    local condition = obfuscator:generateDynamicCondition(seed)
    if condition then
        for i = 1, 5 do
            local nestedCondition = obfuscator:generateDynamicCondition(math_random(1, 10))
...
function CompilerD:createFunctionLiteralExpressionState0(scope, argVar)
...
function CompilerD:createFunctionLiteralExpressionState1(scope, argVar, ifScope)
    return Ast.FunctionLiteralExpression({ Ast.VariableExpression(scope, argVar) }, Ast.Block({
        Ast.IfStatement(
...
function CompilerD:createUpvaluesProxyFunc()
    local scope = Scope:new(self.scope)
    ...
    -- Introduce dynamic variable names and control flow
    local dynamicVarName = "var" .. tostring(math_random(1000, 9999))
    local dynamicScope = Scope:new(scope)
    dynamicScope:addVariable(dynamicVarName)

    if math_random() > 0.5 then
        dynamicScope:addReferenceToHigherScope(self.scope, self.newproxyVar)
    else
        dynamicScope:addReferenceToHigherScope(self.scope, self.setmetatableVar)
    end
...
function CompilerD:createAllocUpvalFunction()
    local scope = Scope:new(self.scope);
...
function CompilerD:emitContainerFuncBody()
    local blocks = {};
...
function CompilerD:freeRegister(id, force)
...
function CompilerD:isVarRegister(id)
    return self.registers[id] == self.VAR_REGISTER;
end

function CompilerD:allocRegister(isVar)
    self.usedRegisters = self.usedRegisters + 1;
...
function CompilerD:isUpvalue(scope, id)
    return self.upvalVars[scope] and self.upvalVars[scope][id];
end

function CompilerD:makeUpvalue(scope, id)
...
function CompilerD:getVarRegister(scope, id, functionDepth, potentialId)
...
function CompilerD:getRegisterVarId(id)
...
function CompilerD:register(scope, id)
    local randomValue = math_random(1, 100)
    if randomValue % 2 == 0 then
        if id == self.POS_REGISTER then
            return self:pos(scope);
        elseif id == self.RETURN_REGISTER then
            return self:getReturn(scope);
        elseif id < MAX_REGS then
        ...
        if id == self.RETURN_REGISTER then
            return self:getReturn(scope);
        elseif id == self.POS_REGISTER then
            return self:pos(scope);
        elseif id >= MAX_REGS then
            local vid = self:getRegisterVarId(MAX_REGS);
            scope:addReferenceToHigherScope(self.containerFuncScope, vid);
...
function CompilerD:registerList(scope, ids)
    local l = {};
    for i, id in ipairs(ids) do
        if math_random(1, 100) % 2 == 0 then
            table.insert(l, self:register(scope, id));
        else
            table.insert(l, self:register(scope, id + math_random(1, 10)));
        end
    end
    return l;
end

function CompilerD:registerAssignment(scope, id)
    if id == self.POS_REGISTER then
...
-- Maybe convert ids to strings
function CompilerD:setRegister(scope, id, val, compundArg)
    if (compundArg) then
...
function CompilerD:setRegisters(scope, ids, vals)
    local idStats = {};
    for i, id in ipairs(ids) do
...
function CompilerD:copyRegisters(scope, to, from)
    local idStats = {};
    local vals    = {};
...
function CompilerD:resetRegisters()
    self.registers = {};
end

function CompilerD:pos(scope)
...
function CompilerD:posAssignment(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.posVar);
    return Ast.AssignmentVariable(self.containerFuncScope, self.posVar);
end

function CompilerD:args(scope)
...
function CompilerD:setReturn(scope, val)
...
function CompilerD:getReturn(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.returnVar);
    return Ast.VariableExpression(self.containerFuncScope, self.returnVar);
end

function CompilerD:returnAssignment(scope)
    scope:addReferenceToHigherScope(self.containerFuncScope, self.returnVar);
    return Ast.AssignmentVariable(self.containerFuncScope, self.returnVar);
end

function CompilerD:setUpvalueMember(scope, idExpr, valExpr, compoundConstructor)
...
function CompilerD:getUpvalueMember(scope, idExpr)
    scope:addReferenceToHigherScope(self.scope, self.upvaluesTable);
    return Ast.IndexExpression(Ast.VariableExpression(self.scope, self.upvaluesTable), idExpr);
end

function CompilerD:compileTopNode(node)
...
function CompilerD:compileFunction(node, funcDepth)
...
function CompilerD:compileBlock(block, funcDepth)
    for i, stat in ipairs(block.statements) do
...
function CompilerD:compileStatement(statement, funcDepth)
    local scope = self.activeBlock.scope;
    -- Return Statement
...
function CompilerD:compileExpression(expression, funcDepth, numReturns)
    local scope = self.activeBlock.scope;
