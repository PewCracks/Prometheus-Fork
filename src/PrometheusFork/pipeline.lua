...
local Enums          = require("PrometheusFork.enums")
local util           = require("PrometheusFork.util")
local Parser         = require("PrometheusFork.parser")
local Unparser       = require("PrometheusFork.unparser")
local logger         = require("logger")

local NameGenerators = require("PrometheusFork.namegenerators")
local Steps          = require("PrometheusFork.steps")
...
local isWindows      = package and package.config and type(package.config) == "string" and
...
function Pipeline:fromConfig(config)
    ...
    for i, step in ipairs(steps) do
        ...
        elseif type(constructor) ~= 'table' then
            logger:error(string.format("The step \"%s\" is not a valid constructor!", step.Name))
        else
...
function Pipeline:setLuaVersion(luaVersion)
    ...
    self.luaVersion = luaVersion -- Update self.luaVersion
end
