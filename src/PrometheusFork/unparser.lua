...
function Unparser:unparse(ast)
	...
	local PrometheusFork = {
		description =
		"This file was generated by PrometheusFork, a Lua obfuscator that provides a wide range of features to protect your code.",
		createHeader = function(self)
			return '--[[ ' ..
			self.description ..
			' ]]--\n';
		end
	...
	local bodyCode = self:unparseBlock(ast.body)
	local customHeader = PrometheusFork:createHeader()
	return customHeader .. bodyCode;
end
...
function Unparser:unparseBlock(block, tabbing)
	...
	for i, statement in ipairs(block.statements) do
		if (statement.kind ~= AstKind.NopStatement) then
			...
			if (self.prettyPrint and not string.sub(statementCode, -1) == ";") then
				statementCode = statementCode .. ";"
			end
...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif (statement.kind == AstKind.WhileStatement) then
		...
		code = "while" ..
			self:whitespaceIfNeeded(expressionCode) .. expressionCode .. self:whitespaceIfNeeded2(expressionCode)
	...
	elseif (statement.kind == AstKind.RepeatStatement) then
		...
		code = "repeat" .. self:whitespaceIfNeeded(bodyCode, self:newline(true))
			...
			"until" .. self:whitespaceIfNeeded(expressionCode) .. expressionCode;
	...
	elseif (statement.kind == AstKind.ForStatement) then
		...
		code = "for" ..
		self:whitespace() .. statement.scope:getVariableName(statement.id) .. self:optionalWhitespace() .. "=";
		...
		code = code ..
			...
			incrementByCode ..
			self:whitespaceIfNeeded2(incrementByCode) .. "do" .. self:whitespaceIfNeeded(bodyCode, self:newline(true))
			.. bodyCode .. self:newline(false)
			.. self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)) .. "end";
	...
	elseif (statement.kind == AstKind.IfStatement) then
		...
		code = "if" ..
			...
			exprcode ..
			self:whitespaceIfNeeded2(exprcode) .. "then" .. self:whitespaceIfNeeded(bodyCode, self:newline(true))
			.. bodyCode;
		...
		if (statement.elsebody) then
			...
			code = code ..
				self:newline(false) ..
				self:whitespaceIfNeeded2(code, self:tabs(tabbing, true)) ..
				"else" .. self:whitespaceIfNeeded(bodyCode, self:newline(true))
				.. bodyCode;
		end
	...
	elseif (statement.kind == AstKind.FunctionDeclaration) then
		...
		code = code ..
		self:newline(false) ..
		bodyCode .. self:newline(false) .. self:whitespaceIfNeeded2(bodyCode, self:tabs(tabbing, true)) .. "end";
	...
	elseif (statement.kind == AstKind.AssignmentStatement) then
		for i, primary_expr in ipairs(statement.lhs) do
			...
			local exprcode = self:unparseExpression(primary_expr, tabbing);
			exprcode = exprcode or "" 
			code = code .. exprcode;
		end
		...
		for i, expr in ipairs(statement.rhs) do
			...
			local exprcode = self:unparseExpression(expr, tabbing + 1);
			exprcode = exprcode or "" 
			code = code .. exprcode;
		end
	...
	elseif self.luaVersion == LuaVersion.LuaU then
		...
		if operator then
			local lhs = self:unparseExpression(statement.lhs, tabbing)
			local rhs = self:unparseExpression(statement.rhs, tabbing)
			lhs = lhs or ""
			rhs = rhs or "" 
			code = code .. lhs .. self:optionalWhitespace() .. operator .. self:optionalWhitespace() .. rhs
		else
			logger:error(string.format("\"%s\" is not a valid unparseable statement in %s!", statement.kind,
				self.luaVersion))
		end
	end
...
function Unparser:unparseExpression(expression, tabbing)
	...
	-- WatermarkExpression
	if (expression.kind == AstKind.WatermarkExpression) then
		-- Do not escape the watermarkExpression
	...
	if (expression.kind == k or expression.kind == AstKind.AssignmentIndexing) then
		...
		-- WatermarkExpression same as StringExpression
		if (expression.index.kind == AstKind.WatermarkExpression and self:isValidIdentifier(expression.index.value)) then
			return base .. "." .. expression.index.value;
		end
	...
	if (expression.kind == k) then
		...
		for i, entry in ipairs(expression.entries) do
			...
			if (entry.kind == AstKind.KeyedTableEntry) then
				...
				code = code ..
				self:optionalWhitespace() ..
				"=" .. self:optionalWhitespace() .. self:unparseExpression(entry.value, tableTabbing);
			else
