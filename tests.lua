...
-- Require PrometheusFork
local PrometheusFork = require("src.PrometheusFork");
...
PrometheusFork.colors.enabled = not noColors;

-- Apply Obfuscation Pipeline
local pipeline = PrometheusFork.Pipeline:new({
...
print("Performing PrometheusFork Tests ...")
...
local presets = PrometheusFork.Presets;
local testdir = "./tests/"
local failed = {};
PrometheusFork.Logger.logLevel = PrometheusFork.Logger.LogLevel.Error;
local fc = 0;
for i, filename in ipairs(scandir(testdir)) do
	...
	print(PrometheusFork.colors("[CURRENT] ", "magenta") .. filename);
	for name, preset in pairs(presets) do
		...
		pipeline = PrometheusFork.Pipeline:fromConfig(preset);
		...
		if funcb == nil then
			print(PrometheusFork.colors("[FAILED]  ", "red") .. "(" .. filename .. "): " .. name .. ", Invalid Lua!");
			...
			if not validated then
				print(PrometheusFork.colors("[FAILED]  ", "red") .. "(" .. filename .. "): " .. name);
				print("[OUTA]    ",    outa);
				print("[OUTB]    ", outb);
				print("[SOURCE]", obfuscated);
				fc = fc + 1;
...
if fc < 1 then
	print(PrometheusFork.colors("[PASSED]  ", "green") .. "All tests passed!");
	return 0;
else
	print(PrometheusFork.colors("[FAILED]  ", "red") .. "Some tests failed!");
	if ciMode then
		error("Test Failed!")
	end
	return -1;
